...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    try {
      // Extract the UUID and data portion
      const matches = data.match(/([0-9a-f-]+):\s*({.*})/i);
      if (!matches) {
        throw new Error('Invalid input format. Expected: UUID: {data object}');
      }

      const [, uuid, jsonString] = matches;
      
      // Parse the JSON portion
      const jsonData = JSON.parse(jsonString);
      
      // Validate input structure
      if (!jsonData.data?.randomNumber || !jsonData.data?.range) {
        throw new Error('Invalid data format');
      }

      const { randomNumber } = jsonData.data;
      const range = {
        min: parseInt(jsonData.data.range.min),
        max: parseInt(jsonData.data.range.max)
      };
